...
// ── Operational counter evidence (M9-A) ──────────────────────────────────────
...
test "ops: lifecycle attempt/success/failure boundaries" {
    ...
    // start_attempts increments regardless of outcome
    ...
    // AlreadyStarted increments attempts but not successes or failures
...
test "ops: queue feed accepted/rejected and apply drain accounting" {
    ...
    // apply drains and tracks bytes_applied
    ...
    // apply on empty queue still increments apply_calls; bytes_applied unchanged
    _ = s.apply();
    try std.testing.expectEqual(@as(u32, 2), s.ops.apply_calls);
    try std.testing.expectEqual(@as(u64, 5), s.ops.bytes_applied);

    // bytes_fed accumulates across multiple accepted feeds
    try s.feed("ab");
    try s.feed("cd");
...
test "ops: reset_calls accounting" {
    ...
    // reset does not affect feed/apply counters
    try s.feed("data");
    s.reset();
    try std.testing.expectEqual(@as(u32, 1), s.ops.feed_accepted);
...
test "ops: resize/control accounting" {
    ...
    // valid resize increments resize_valid_calls
    try s.resize(100, 40);
    ...
    // invalid resize (zero dim) increments resize_invalid_calls
    ...
    // transport error on resize increments resize_transport_errors (dims already committed)
    var sf = try Session.init(.{
        .allocator = std.testing.allocator,
        .cols = 80,
        .rows = 24,
    ...
    // control_calls increments on every control()
...
test "ops: counters accumulate across lifecycle transitions" {
    ...
    // Restart — counters accumulate, not reset
    try s.start();
    try s.feed("de");
    _ = s.apply();
    s.stop();
...
// ── Snapshot/restore evidence (M10-A) ────────────────────────────────────────
...
test "snapshot/restore: transport attachment unchanged by restore" {
    ...
    // transport still attached and functional after restore
    try s.start();
    try std.testing.expectEqual(SessionStatus.active, s.status);
    s.stop();
}
