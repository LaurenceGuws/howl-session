...
```bash
rg "app_logger|publication|ffi|android|platform|editor|workspace" --glob '*.zig' src
```

Expected: zero matches unless intentionally introduced by an approved ticket scope.
